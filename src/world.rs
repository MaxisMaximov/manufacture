...
pub struct TEMPLATE_wrCell {
    pub c_char: char,
    pub c_color: TYPE::colorSet,
}
impl TEMPLATE_wrCell {
    pub fn new() -> Self {
        TEMPLATE_wrCell {
            c_char: ' ',
            c_color: MISC::COLORS::COLORS_DEF,
        }
    }
    ...
    pub fn new() -> Self {
        TEMPLATE_wChunk {
            ch_cells: [TEMPLATE_wrCell::new();
                WORLD::GENERAL::WORLD_CHUNK_X * WORLD::GENERAL::WORLD_CHUNK_Y],
        }
    }
    pub fn newDummy() -> Self {
        TEMPLATE_wChunk {
...
impl Index<TYPE::vector2> for TEMPLATE_wChunk {
...
impl IndexMut<TYPE::vector2> for TEMPLATE_wChunk {
...
impl TEMPLATE_world {
    ...
    fn w_util_circleIter(
        ...
        INw_initPos: TYPE::vector2,
        INw_iters: usize,
        INw_size: Range<usize>,
    ) -> Vec<TYPE::vector2> {
        let mut w_genCircleTiles: Vec<TYPE::vector2> = Vec::new();

        // Set variables to be used by iterations
        let mut w_iterCoords: TYPE::vector2 = INw_initPos;
        let mut w_nextIterCoords: TYPE::vector2;
        ...
        for _ in 0..INw_iters {
            ...
            let w_circleStartingPosition: TYPE::vector2 = (
            ...
            w_nextIterAreaX = w_iterCoords.0.saturating_sub(w_iterRadius)
                ..(w_iterCoords.0 + w_iterRadius).clamp(0, WORLD::GENERAL::WORLD_GRID_X);
            w_nextIterAreaY = w_iterCoords.1.saturating_sub(w_iterRadius)
                ..(w_iterCoords.1 + w_iterRadius).clamp(0, WORLD::GENERAL::WORLD_GRID_X);
            loop {
                w_nextIterCoords = (
    ...
    pub fn w_generateRandom(&mut self) {
        ...
        let mut w_genLakeTiles: Vec<TYPE::vector2> = Vec::new();
        for _ in 0..w_RNG.gen_range(WORLD::GENERATION::GEN_POND_ITERS) {
        ...
        // Forests
        let mut w_genForestTiles: Vec<TYPE::vector2> = Vec::new();
    ...
    pub fn w_returnChunkArray(&self, w_centerCoords: TYPE::vector2, INw_radius: usize) -> Vec<&TEMPLATE_wChunk> {

        // Calc size quickly
        let w_size: usize = INw_radius * 2 + 1;

        // Set positions
        let w_startPosition: TYPE::vector2 = (
            w_centerCoords.0.saturating_sub(INw_radius),
            w_centerCoords.1.saturating_sub(INw_radius),
        );

        // WORLDSIZE - 1 to prevent overflows
        let w_endPosition: TYPE::vector2 = (
        ...
        for XPOS in w_startPosition.0..=w_endPosition.0 {
            ...
            for YPOS in w_startPosition.1..=w_endPosition.1 {
                OUTw_chunkVec[w_vecX + w_vecY * w_size] =
                    &self.w_chunks[XPOS + YPOS * WORLD::GENERAL::WORLD_X];
                w_vecY += 1
            }
...
impl Index<TYPE::vector2> for TEMPLATE_world {
    type Output = TEMPLATE_wrCell;

    fn index(&self, index: TYPE::vector2) -> &Self::Output {
        &self.w_chunks[index.0 / WORLD::GENERAL::WORLD_CHUNK_X
            + index.1 / WORLD::GENERAL::WORLD_CHUNK_Y * WORLD::GENERAL::WORLD_X][(
            index.0 % WORLD::GENERAL::WORLD_CHUNK_X,
            index.1 % WORLD::GENERAL::WORLD_CHUNK_Y,
        )]
    }
}
impl IndexMut<TYPE::vector2> for TEMPLATE_world {
    fn index_mut(&mut self, index: TYPE::vector2) -> &mut Self::Output {
        &mut self.w_chunks[index.0 / WORLD::GENERAL::WORLD_CHUNK_X
            + index.1 / WORLD::GENERAL::WORLD_CHUNK_Y * WORLD::GENERAL::WORLD_X][(
            index.0 % WORLD::GENERAL::WORLD_CHUNK_X,
            index.1 % WORLD::GENERAL::WORLD_CHUNK_Y,
        )]
